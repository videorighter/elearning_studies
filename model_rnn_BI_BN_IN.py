...
def model_execution(X_train, X_val, X_test, y_train, y_val, y_test, num, args):
    ...
    if os.path.isdir('./rnn_models/'):
        try:
            os.rmdir('./rnn_models/')
        except OSError:
            shutil.rmtree('./rnn_models/')
        os.mkdir('./rnn_models/')
    else:
        os.mkdir('./rnn_models/')

    MODEL_SAVE_FOLDER_PATH = './rnn_models/'
    ...
    # load best model
    best_model_path = sorted(glob.glob('./rnn_models/*.hdf5'), key=os.path.getctime)
    ...
    if os.path.isfile("./rnn_models/rnn_result.txt"):
        os.remove('./rnn_models/rnn_result.txt')
        with open("./rnn_models/rnn_result.txt", 'w') as f:
...
def draw_plot(y_test, y_pred, cm, num, history):
    ...
    fig.suptitle('Vanilla RNN classifier', fontsize=23)
    ...
    ax1.set_title('Accuracy/Loss plot for Vanilla RNN classifier', fontsize=20)
    ...
    ax2.set_title('ROC curve for concentration Vanilla RNN classifier', fontsize=20)
    ...
    ax3.set_title('Confusion matrix for Vanilla RNN classifier', fontsize=20)
    ax3.tick_params(axis='both', labelsize=15)
    sn.heatmap(df_cm, annot=labels, fmt='', annot_kws={"size": 20}, cmap='Blues')

    plt.savefig(f"./rnn_models/rnn_{num + 1}.png")
    plt.show()
...
def summarize_results(scores):
    ...
    if os.path.isfile("./rnn_models/rnn_result.txt"):
        with open("./rnn_models/rnn_result.txt", 'a') as f:
...
def main():
    ...
    parser.add_argument('--device', type=str, default='rnn', help='Select device to execute')
