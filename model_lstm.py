...
def model_execution(X_train, X_val, X_test, y_train, y_val, y_test, num, args):
    ...
    MODEL_SAVE_FOLDER_PATH = f'./lstm_models_{args.rnn_layer}_{args.ff_layer}_{args.epochs}_{args.batch_size}_{args.drop_out}_{args.lr}/'
    ...
    if not args.plot_only:
        ...
        filename = MODEL_SAVE_FOLDER_PATH + 'LSTM_{epoch:02d}-{val_loss:.4f}_' + f'{num + 1}' + '.hdf5'
    ...
    if os.path.isfile(MODEL_SAVE_FOLDER_PATH + "lstm_result.txt"):
        os.remove(MODEL_SAVE_FOLDER_PATH + 'lstm_result.txt')
        with open(MODEL_SAVE_FOLDER_PATH + "lstm_result.txt", 'w') as f:
            f.write(f"LSTM {num + 1}'s execution \n"
                    f"confusion matrix: \n {cm} \n"
                    f"accuracy: \n {accuracy} \n"
...
def draw_plot(y_test, y_pred, cm, num, history, args):
    ...
    if not args.plot_only:
        ...
        plt.title('Accuracy/Loss plot for LSTM classifier', fontsize=20)
        ...
        plt.grid(True)
        plt.savefig(MODEL_SAVE_FOLDER_PATH + f"lstm_{num + 1}_loss.png")
    ...
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    plt.title('ROC curve for concentration LSTM classifier', fontsize=20)
    plt.xlabel('False Positive Rate (1 - Specificity)', fontsize=17)
    plt.ylabel('True Positive Rate (Sensitivity)', fontsize=17)
    plt.tick_params(axis='both', labelsize=14)
    plt.legend(loc="lower right", fontsize=17)
    plt.grid(True)
    plt.savefig(MODEL_SAVE_FOLDER_PATH + f"lstm_{num + 1}_auroc.png")

    plt.figure(figsize=(10, 8))
    plt.title('Confusion matrix for LSTM classifier', fontsize=20)
    plt.tick_params(axis='both', labelsize=14)
    sn.heatmap(df_cm, annot=labels, fmt='', annot_kws={"size": 20}, cmap='Blues')

    plt.savefig(MODEL_SAVE_FOLDER_PATH + f"lstm_{num + 1}_cm.png")
...
def summarize_results(scores, args):
    ...
    if os.path.isfile(MODEL_SAVE_FOLDER_PATH + "lstm_result.txt"):
        with open(MODEL_SAVE_FOLDER_PATH + "lstm_result.txt", 'a') as f:
