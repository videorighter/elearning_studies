...
def model_execution(X_train, X_val, X_test, y_train, y_val, y_test, num, args):
    ...
    if os.path.isdir('./lstm_models/'):
        try:
            os.rmdir('./lstm_models/')
        except OSError:
            shutil.rmtree('./lstm_models/')
        os.mkdir('./lstm_models/')
    else:
        os.mkdir('./lstm_models/')

    MODEL_SAVE_FOLDER_PATH = './lstm_models/'
    ...
    # load best model
    best_model_path = sorted(glob.glob('./lstm_models/*.hdf5'), key=os.path.getctime)
    ...
    if os.path.isfile("./lstm_models/lstm_result.txt"):
        os.remove('./lstm_models/lstm_result.txt')
        with open("./lstm_models/lstm_result.txt", 'w') as f:
...
def draw_plot(y_test, y_pred, cm, num, history):
    ...
    fig.suptitle('LSTM classifier', fontsize=23)
    ...
    ax1.set_title('Accuracy/Loss plot for LSTM classifier', fontsize=20)
    ...
    ax2.set_title('ROC curve for concentration LSTM classifier', fontsize=20)
    ...
    ax3.set_title('Confusion matrix for LSTM classifier', fontsize=20)
    ax3.tick_params(axis='both', labelsize=15)
    sn.heatmap(df_cm, annot=labels, fmt='', annot_kws={"size": 20}, cmap='Blues')

    plt.savefig(f"./lstm_models/lstm_{num + 1}.png")
    plt.show()
...
def summarize_results(scores):
    ...
    if os.path.isfile("./lstm_models/lstm_result.txt"):
        with open("./lstm_models/lstm_result.txt", 'a') as f:
